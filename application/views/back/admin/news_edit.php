...
foreach ($news_data as $row) {
    ...
    <div class="row">
        <div class="col-md-12">
            <?php
            echo form_open(base_url() . 'admin/news/update/' . $row['news_id'], array(
            ...
            <div class="panel-body">
                                    ...
                                    <div class="col-sm-12">
                                        <center>
                                            <div class="col-sm-12" style="padding:10px;">
                                                <?php
                                                if (file_exists('uploads/news_image/' . $row1['thumb'])) {
                                                    ?>
                                                    <img class="img-responsive img-border blah"  src="<?php echo base_url(); ?>uploads/news_image/<?php echo $row1['thumb']; ?>?t=<?= time(); ?>" style="width:100%;"  >
                                                    <?php
                                                } else {
                                                    ?>
                                                    <img class="img-responsive img-border blah"  src="<?php echo base_url(); ?>uploads/news_image/default.jpg" style="width:100%;" >
                                                    <?php
                                                }
                                                ?>
                                            </div>
                ...
                <div class="form-group btm_border">
                    ...
                    <div class="col-sm-10">
                        <?php echo $this->Crud_model->select_html('news_category', 'news_category', 'name', 'edit', 'demo-chosen-select required', $row['news_category_id'], '', '', 'get_cat'); ?>
                    </div>
                </div>
                <div class="form-group btm_border" id="sub">
                    ...
                    <div class="col-sm-10" id="sub_cat_db">
                        <?php echo $this->Crud_model->select_html('news_sub_category', 'news_sub_category', 'name', 'edit', 'demo-chosen-select', $row['news_sub_category_id'], '', '', '') ?>
                    </div>
                ...
                <div class="form-group btm_border">
                    ...
                    <div class="col-sm-10">
                        <?php
                        echo $this->Crud_model->select_html('news_reporter', 'news_reporter', 'name', 'edit', 'demo-chosen-select required', $row['news_reporter_id']);
                        ?>
                    </div>
                </div>
                <div class="form-group btm_border">
                    ...
                    <div class="col-sm-6">
                        <input class='aiz_switchery1' value="published" type="checkbox" name="status" id="id2"
                               ...
                               data-tm='<?php echo translate('news_published'); ?>' 
                               data-fm='<?php echo translate('news_unpublished'); ?>' 
                ...
                <div class="form-group btm_border">
                    ...
                    <div class="col-sm-10">
                        <input class="aiz_switchery1" value="ok" type="checkbox" name="breaking_news" data-set="breaking_news" id="id2" 
                               data-id='<?php echo $row['news_id']; ?>' 
                               data-tm='<?php echo translate('added_to_breaking_news'); ?>' 
                               data-fm='<?php echo translate('removed_from_breaking_news'); ?>' 
                               <?php if ($row['breaking_news'] == 'ok') { ?>checked<?php } ?> />  
                    </div>
                </div>
            </div>
                ...
                <div class="row">
                    <div class="col-md-11">
                        <span class="btn btn-purple btn-labeled fa fa-refresh pro_list_btn pull-right" 
                              onclick="ajax_set_full('edit', '<?php echo translate('edit_news'); ?>', '<?php echo translate('successfully_edited!'); ?>', 'news_edit', '<?php echo $row['news_id']; ?>')"><?php echo translate('reset'); ?>
                        </span>
                    </div>
                    <div class="col-md-1">
                        <span class="btn btn-success btn-md btn-labeled fa fa-upload pull-right enterer" onclick="form_submit('news_edit', '<?php echo translate('successfully_edited!'); ?>'); proceed('to_add');" ><?php echo translate('edit'); ?></span>
                    </div>
                </div>
            </div>                   
        ...
        $('body').on('click', '.remove_data', function () {
            ...
            setTimeout(function () {
                bootbox.confirm('Really Want to Delete this Image?', function (result) {
                    if (result) {
                        $.ajax({
                            url: "<?php echo base_url(); ?>admin/news/delete_img/<?php echo $row['news_id']; ?>/" + img_name,
                            beforeSend: function () {
...
<script>
    ...
    function get_cat(id) {
        $('#sub').hide();
        $('#sub_cat_db').html('');
        ajax_load(base_url + 'admin/news/sub_by_cat/' + id, 'sub_cat', 'other');
    }
    function other() {
        set_select();
        $('#sub').show('slow');
    ...
    function save_draft() {
        ...
        setTimeout(form_submit('news_edit', '<?php echo translate('successfully_edited!'); ?>'), 500);
    }
    function set_summer() {
