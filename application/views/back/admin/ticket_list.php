    ...
    <table id="demo-table" class="table table-striped"  data-pagination="true" data-show-refresh="true" data-ignorecol="0,2" data-show-toggle="true" data-show-columns="false" data-search="true" >
        ...
        <thead>
            <tr>
                ...
                <th><?php echo translate('requested_time'); ?></th>
            ...
            foreach ($tickets as $row) {
                ...
                <tr>
                        ...
                        if ($from['type'] == 'user') {
                            ...
                            <a class="btn btn-mint btn-xs btn-labeled fa fa-location-arrow" data-toggle="tooltip" 
                               onclick="ajax_modal('view_user', '<?php echo translate('view_profile'); ?>', '<?php echo translate('successfully_viewed!'); ?>', 'user_view', '<?php echo $from['id']; ?>')" data-original-title="View" data-container="body">
                                   <?php echo $this->db->get_where('user', array('user_id' => $from['id']))->row()->firstname . ' ' . $this->db->get_where('user', array('user_id' => $from['id']))->row()->lastname; ?>
                            </a>
                            <?php
                        } else {
                            ?>
                    ...
                    <td>
                        ...
                        if ($num > 0) {
                            ?>
                            <span class="btn btn-mint btn-xs btn-labeled " style="margin:2px; margin-left:10px;"><?php
                                echo translate('new') . ' ' . '(' . ' ';
                                echo $num . ' ' . ')';
                                ?></span>
                            <?php
                    ...
                    <td><?php echo date('d M, Y h:i a', $row['time']); ?></td>
                    <td class="text-right">
                        <a class="btn btn-info btn-xs btn-labeled fa fa-location-arrow" data-toggle="tooltip" 
                           onclick="ajax_set_full('view', '<?php echo translate('view_contact_ticket'); ?>', '<?php echo translate('successfully_viewed!'); ?>', 'contact_ticket_view', '<?php echo $row['ticket_id']; ?>');
                                   proceed('to_list');"
